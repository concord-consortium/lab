...
describe("UnitsTranslation module", () => requirejs(['models/md2d/models/units-translation'], function(UnitsTranslation) {
  ...
  for (var unitType in expectedMD2DUnitValueInMKSUnits) {
    ...
    it(`should translate ${unitType} correctly into MD2D units`, () => translation.translateToModelUnits(value, unitType).should.be.approximately(1, 1e-3));

    it(`should translated ${unitType} correctly from MD2D units`, () => translation.translateFromModelUnits(1, unitType).should.be.approximately(value, value * 1e-3));
  }

  return it("should return an untranslated value for unit types it does not convert", function() {
    translation.translateToModelUnits(1, 'made up unit type').should.eql(1);
    return translation.translateFromModelUnits(1, 'made up unit type').should.eql(1);
  });
}));
