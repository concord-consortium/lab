...
helpers.withIsolatedRequireJSAndViewsMocked(function(requirejs) {
  ...
  return describe("Lab interactives: serialization", function() {
    ...
    before(() => // Overwrite default float arrays type. It's required, as when Float32Array is used,
    // there are some numerical errors involved, which cause that serialize-deserialize
    // tests fail.
    arrayTypes.floatType = "Float64Array");

    return describe("serialization right after initialization should return object equal to original JSON input", function() {
          ...
          if (fs.statSync(inputFile).isDirectory()) {
            ...
            it(`testing: ${inputFile}`, function() {
              ...
              if (interactive.models[0].url) {
                ...
                helpers.withModel(modelObject, () => controller = interactivesController(interactive, 'body'));
              } else {
