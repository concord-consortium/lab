import GeneticEngine from "../../../src/lab/models/md2d/models/engine/genetic-engine";
  ...
  describe("[basic tests of the class]", function () {
    describe("GeneticEngine constructor", function () {
      ...
      it("should act as a constructor", function () {
        const geneticEngine = new GeneticEngine(new Model({}));
        should.exist(geneticEngine);
      });
    });

    describe("GeneticEngine instance", function () {
      let model = null;
      ...
      const checkDNAArray = function (array, sequence) {
        const offset = geneticEngine.PRECODING_LEN;
        return (() => {
          const result = [];
          for (let i = 0, end = sequence.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
            array[i + offset].idx.should.eql(i + offset);
            result.push(array[i + offset].type.should.eql(sequence[i]));
          }
          return result;
        })();
      };

      const checkMRNAArray = (array, sequence) => (() => {
        const result = [];
        for (let i = 0, end = sequence.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
          array[i].idx.should.eql(i);
          result.push(array[i].type.should.eql(sequence[i]));
        }
        return result;
      })();
      ...
      it("should perform single step of DNA to mRNA transcription", function () {
        ...
        checkMRNAArray(geneticEngine.viewModel.mRNA, "AU");
      }); // so it's added to mRNA.
      ...
      it("shouldn't let the user perform any mutation outside the DNA coding region", function () {
        model.set("DNA", "ATGC");
        const DNALen = model.get("DNA").length;
        const offset = geneticEngine.PRECODING_LEN;
      ...
      it("should provide stateBefore() and stateAfter() helper methods", function () {
        model.set("DNAState", "intro-cells");
