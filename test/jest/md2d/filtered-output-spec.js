...
import RunningAverageFilter from "../../../src/lab/common/filters/running-average-filter";
import Model from "../../../src/lab/models/md2d/models/modeler";

describe("MD2D filters", () => {
  describe("RunningAverageFilter", function () {
    let runningAvg = null;
    ...
    describe("[basic tests of the class]", function () {
      ...
      describe("when new points are being added", function () {
        ...
        it("should calculate running average - complex test case with linear interpolation", function () {
          runningAvg.addSample(0, 0);
          runningAvg.calculate().should.eql(0);
    ...
    describe("[tests in the MD2D modeler context]", function () {
      let model = null;
      ...
      describe("when filter is attached to output property", function () {
        ...
        it("should follow changes of the current model step", function () {
          model.tick();
          model.tick();
          model.tick();
          model.get("time").should.equal(3);
      ...
      describe("when filter is attached to a parameter", function () {
        beforeEach(function () {
          model = new Model(simpleModel);
          // Filter: 'test', period: 2fs.
