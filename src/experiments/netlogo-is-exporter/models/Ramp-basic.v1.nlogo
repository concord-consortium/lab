...
vehicles-own [x-val y-val speed frict mass offset size-100 spring-left? spring-right?]       ; x-val y-val are the x,y locations. 
...
to go              ; this is the main forever button
  if starting? [
    ...
    initialize
    set starting? false]
  if running? [run-vehicles]  ; computes the motion of the vehicles
  every .1 [
    act-on-changes
    support-mouse
    update-displays]
...
to update-displays
  set height ""
  set mass-selected ""
  if blinking? [                ; if the selected vehicle is supposed to blink
    set height [y-val] of vehicle selected-vehicle
    set mass-selected [mass] of vehicle selected-vehicle
    if not old-blinking? [      ; capture the first time blinking? became true
;      show-options              ; hide the graph and shows options for the blinking vehicle
      set old-blinking? true]
    ask vehicle selected-vehicle [
      
      let loc uv let u first loc let v last loc
      if in-wind? u v [
        st if ((int (10 * timer)) mod 4) = 1 [ht]]]]      
  if old-blinking? and not blinking? [ ;detect when blinking is first turned off
    switch-focus-to 2
    rescale-grid
    switch-focus-to 1
    ask vehicle selected-vehicle [  ; if it is in the window, turn it on
      let loc uv
      if in-wind? first uv last uv [st]]
    set old-blinking? false]

  if running? and not empty? v-who [  ; draw a dot on the graph for each vehicle
    let i 0
    while [i < length v-who] [
      let c item i vehicle-colors ; color of this vehicle
...
to initialize
  ...
  set vehicle-colors [red green yellow blue magenta orange]
...
to run-vehicles
  ...
  ask vehicles [
    ...
    set time time + dt
    ifelse speed > 0 
        [set a g * ( sin heading  - friction * ch) ] ; the accleration to the right
        [set a g * ( sin heading  + friction * ch) ] ; the acceleration to the left
...
;    switch-focus-to 1
    ...
    set color item n vehicle-colors
...
Y-Axis
Y-Axis
"Distance (m)" "Speed (m/s)" "Momentum (kg-m/s)" "Potential Energy (J)" "Kinetic Energy (J)" "Total Energy (J)"
0
...
586
55
Mass of selected vehicle
word (precision (1000 * Mass-selected) 2) \" g\"
17
1
11
