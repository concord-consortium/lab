...
;New in v18. Feb 3, 2013. Based on v15h. 
;     New dynamics. The airbag now has two modes. If the dummy contacts it during inflation, the airbag
;        pushes hard to the right. If it fully inflates before contact, it stays inflated until it is contacted. 
;        The airbag then deflates while pushing back and staying in contact with the dummy. 
;        When the dummy stops, the airbag then self-deflates. 
;     Filters that permit all runs of the same kind (safe, not, maybe) to be graphed together. 

;New in v15h Dec 23, 2012
;    Changed the dymaics so that the airbag gives a force only to the right when the dummy is going left. 
...
to initialize
  ...
  set dot-size .5            ; sets the size of each dota
  set min-x 0
  ...
  ; set slider defaults
  set distance-to-steering-wheel .3
...
to on/off    ; this is the main execution loop
  ...
  if mouse-inside? [support-mouse]   ; not currently supported
  tick
end
...
to support-mouse      ; incomplete
  if in-grid? mouse-xcor mouse-ycor [  ; 
    ]                                  ; unfinished
  let ig2? in-grid2? mouse-xcor mouse-ycor
  if ig2? and (not old-in-grid2?) [    ; if the mouse is in the grid2 but was not last time
    set old-in-grid2? true             ; reset flag
    ask dots [set size 0]              ; dim the dots and lines
    ask lines [set thickness .05 ]]
  if (not ig2?) and old-in-grid2? [
    set old-in-grid2? false            ; reset flag
    ask dots [set size dot-size]       ; restore the brightness of the dots and lines on leaving grid2
    ask lines [set thickness .5 ]]
  if ig2? [                            ; now thicken runs that are near the cursor
    let i 1                            ; check each run 
    while [i <= run-number ][
      ifelse cursor-near? i  
        [ ask dots with [run-n = i] [set size 5 * dot-size]] ; brighten the dots
        [ ask dots with [run-n = i] [set size 0]] ; dim the dots 
      set i i + 1]]
end
...
to run-airbag ; computes and draws the position or velocity graphs of the airbag and dummy 
  ...
  setup-for-run   ; housekeeping
  package-data  ; package up the variables used in analyzing student patterns
                   ; it may seem odd to place this here, but it is only here that the code can know that the student is
                   ;       done looking at the last run. We want to capture everything that the student has done....
...
to package-data
  ...
  if run-number > 1 [  ; Ignore data storage at the start of the first run. (there was no prior run to record!)
    ...
    ; run-data is a list of objects, each of which is all the data from a run. 
    
  ; now prepare for the current run
  ...
  reset-timer      ; the timer is used to record how long the student spent on this run. 
end
...
to-report bags-force [t x-dum v-dum dt]          ; calculate the bag's dynamics: x-bag, v-bag, and the bag's force on the dummy given the dummy at x-dum
                                                 ;    x-bag and v-bag are globals. Only the force is reported. 
  ...
  let penetrating? ((x-dum < x-bag) and ((x-bag - x-dum) < penetration ))  ; if true the dummy has partly penetrated the bag  
  let in-contact? x-dum <= x-bag                 ; if the dummy is to the left of the bag, they are in contact. 
  ...
  if (in-contact? or penetrating?) and t <= t-start-inflate [      ; if the bag and dummy are in contact, but the bag hasn't even begun to inflate
    set dummy-crashed? true                      ; the dummy has just crashed
    set x-bag 0 set v-bag 0 report 0 ]  
  if in-contact? and inflating? [                ; if the bag and dummy are in contact and the bag is inflating
    if penetrating? [                            ; if the dummy is penetrating the bag...
      set v-bag airbag-size / time-to-fill-bag
      report stiffness-inflating * (x-bag - x-dum ) / penetration ] ; report a fraction of the penetration
    if not penetrating? [                        ; must have penetrated as far as it can go
    set x-bag x-dum + penetration                ; the new bag location is the same as the dummy's plus the penetration
    set v-bag v-dum             
    report stiffness-inflating ]]   ; use the inflating stiffness--a force
  if in-contact? and t >= t-fully-inflated [     ; if the bag and dummy are in contact and the bag is already fully inflated
    if penetrating? [
      set v-bag airbag-size / time-to-fill-bag
      report stiffness-inflated * (x-bag - x-dum ) / penetration ] ; report a fraction of the penetration      
    if not penetrating? [
      set x-bag x-dum + penetration
      set v-bag v-dum                         
      report stiffness-inflated ]]                 ; use the already-inflated stiffness--a force
  if not in-contact? [
    ifelse inflating?                            ; if the bag is inflating
      [ set v-bag airbag-size / time-to-fill-bag ; calculate the velocity of the bag
        set x-bag x-bag + v-bag * dt ]           ; move the airbag along 
      [ set v-bag 0 ]                            ; if it is not inflating, set v-bag to zero and don't change x-bag
    report 0 ]
end
...
;  ask dots with [run-n = run-number ] [
   ask dots [ 
;   st           ; if there are any dots, set them according to their position or velocity
...
to show-results [a-max]       ; generates a line in the output box summarizing a run
  ...
  if a-max < .95 * a-cutoff  [set dummy-status "Yes"]  ; give a 10% margin 
  if a-max > 1.05 * a-cutoff  [set dummy-status "No"]
...
to display-run [n]
  if n > 0 and n <= run-number [
    ask dots with [run-n = n ][
      ...
      let pw prior-who
      if pw > 0 [
        create-line-with dot pw       ; prior-who is a dot variable that gives the who of the previous point
        ask line who pw [
          set hidden? false
          set thickness .5
          set color c]]]
...
to display-runs-colored [clr]
  let i 1
  while [i <= run-number ][
    let c [dot-color] of one-of dots with [run-n = i and dot-color != white]  ; the only way to get the color of run i
    if c = clr [
      ask dots with [run-n = i ] [
        st ; shows all the dots in that run, even the white ones.
          ; reconnect the dots
        let pw prior-who
        let dc dot-color
        if pw > 0 [
