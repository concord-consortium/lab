...
;  set dummy-colors ["red" "orange" "green" "cyan" "yellow" "magenta" "gray" "brown" "lime" "turquoise" "sky" "violet" "pink" "blue"] 
 ...
 create-messages 1 [
   ...
   set color white
   setxy .35 * min-pxcor .97 * max-pycor ht]
...
to run-airbag ; computes and draws the position or velocity graphs of the airbag and dummy 
  ...
  let d-color cyan; selects the color for this run
  ask actors   [st]
...
;    "What airbag deployment time best keeps drivers safe in low speed collisions?"]
  ...
  ask messages with [number = 0 ][ st ]
  tick
  while [what-is-your-question? = "" ][ wait .1]
  set the-question what-is-your-question?
  ask messages with [number = 0 ][ ht ]
  tick
  ...
  let a-psudo 0
  if time-to-stop != 0 [set a-psudo 0 - (car-speed / time-to-stop)]     ; this is the apparent acceleration of the dummy due to the car crash
  ...
  while [t <= experiment-duration-in-sec][        ; begin the calculation loop
    ...
    if x-dum < x-bag [                             ; if the dummy is inside the bag...
      ifelse t < time-to-fill-bag 
        [set a-dum a-dum + const * ( 1 - (x-dum / x-bag ))]   ; add in the force of the airbag while inflating
        [set a-dum a-dum + const2 * ( 1 - (x-dum / x-bag ))]] ; add in the force of the airbag after inflating 
...
1880
63
Const sets the stiffness of the airbag during inflation
11
0.0
1
...
1858
123
Const2 sets the stiffness during deflation
11
0.0
1
