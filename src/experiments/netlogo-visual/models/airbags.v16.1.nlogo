...
; New in v16.1 Oct 23
;     Renamed and corrected JSON export functions:
;     - DataSeriesDescription
;     - ModelDataDescription
;     - DataSeries

; New in v16 Oct 15
;     A new On/off button eliminates the need for four other buttons
;     User must pick a question before changing variables and before running. 
;     Time series of runs are initially graphed in white, but then permanently change to red, green, or yellow
;     Added some new reports--counts of repeated experiments, one-variable only changed, etc. 
...
;  dummy-status  ; tells whether the dummy survives
  vertical-axis-type
  old-max-y       ; used to restore the value in the user input box when swapping between position and velocity
  ...
  time-to-stop           ; this will be the max-time-to-stop modified by (car-speed / 40) so that it is proportional to car speed. 
  starting-up?
  old-axis
  old-vertical-axis
  old-horizontal-axis
  ...
  variables-locked?        ; if true, students cannot change variables and an attept to do so will trigger the question-setting popup. 
  running?                 ; true during a run--blocks the question setting if, by chance, students try to change a variable. 
  
  ModelDataDescription
  DataSeries
  data-ready?
  dum-data             ; a list of t,x,v triplets for the dummy
...
dots-own [ x-val y-val velocity selected? removed? dot-color run-n prior-who]         
...
to on/off    ; the idle loop that checks for changes in the UI.
  if starting-up? [
    ask dots [die]
...
to initialize    
  ...
  reset-timer 
  output-print (word "Run Survives   Max-acc  Car speed   Distance  Bag size  Fill time")
  set run-number 0
  set run-data [ ]      ; this global stores the results of runs. Each run creates five values, the four parameters and survival
  set vertical-axis-type axis
  set old-axis axis
  set old-vertical-axis vertical-axis
  set old-horizontal-axis horizontal-axis
  set old-enter-a-run-number enter-a-run-number 
  reset-old-variables     ; set 'old' version of each variable to its current value
  set variables-locked? true       ; users cannot change sliders. 
  set running? false        ; indicates whether user has changed slider values.
  set axis "Position (m)"
  ...
  set N-points 100          ; the number of points in each dataset
  ...
  make-ModelDataDescription   ; needed to communicate with the outside world
  set data-ready? true
 ...
 ; first create the larger actors for the stage
  create-actors 1 [               ; create an airbag 
    ...
    set location "stage" set shape "circle" ]
  create-actors 1 [                              ; create a large headless dummy for the stage
    set size actor-size set number 2 
    set location "stage" set shape "dummy-body" 
    set color red ] ; for the first run, the dummy is red
  
 create-actors 1 [                              ; create a dummy head
    set size actor-size set number 3 
    set location "stage" set shape "dummy-head" 
    set heading 0
    set color red ] ; for the first run, the dummy is red
  
  ; next create smaller ones that move on the graphs--start hidden and show only to animate graph
  create-actors 1 [set size .6 * actor-size 
    set number 1  ;--first the airbag
    set location "grid" set shape "circle" 
    set color read-from-string "white" 
    ht]
  create-actors 1 [set size .6 * actor-size ; now the dummy body
    set number 2 
    set location "grid" set shape "dummy-body"  
    set color red 
    ht]
  create-actors 1 [set size .6 * actor-size ; now the dummy head
    set number 3 
    set location "grid" set shape "dummy-head"  
...
to look-for-changes   ; look for changes in the sliders and selectors, take appropriate action 
  ; in the on/off forever loop
  let change-detected? false
  if old-axis != axis [
    set old-axis axis    ; handle a change of axis
    set vertical-axis-type axis
    ask dots [ht]   
    draw-graph
    set graph-type-used 3]; if the student changes the axis, this reports that both types were used
  ...
  if old-distance-to-steering-wheel != distance-to-steering-wheel [
     ifelse variables-locked?          ; if the variables are locked... 
       ...
       [set old-distance-to-steering-wheel distance-to-steering-wheel ; if not locked...
         ...
         draw-actor-on-stage 2 distance-to-steering-wheel 0 blue
         draw-actor-on-stage 3 distance-to-steering-wheel 0 blue]]
  
  if old-enter-a-run-number != enter-a-run-number [  ; detect a change in the run-number input...
     set old-enter-a-run-number enter-a-run-number
     show-run]                                       ; and show that run
  
  if car-speed != old-car-speed and variables-locked? [
    set car-speed old-car-speed
    set change-detected? true]
  
  if old-airbag-size != airbag-size and variables-locked? [
    set airbag-size old-airbag-size 
    set change-detected? true]
  
  if old-time-to-fill-bag != time-to-fill-bag and variables-locked? [
    set time-to-fill-bag old-time-to-fill-bag 
    set change-detected? true]
  
  if change-detected? and not running? [pick-a-question]
end
...
to add-position-point [pos time cnum]
  create-dots 1 [                    ; create a new position dot
    ...
    set selected? true set removed? false  place x-val y-val st] ; show on screen
...
to place-actors [clr]; sets actors at their initial positions
    ...
    draw-actor-on-stage 2 distance-to-steering-wheel 0 blue; place the dummy body
    draw-actor-on-stage 3 distance-to-steering-wheel 0 blue; place the dummy head
end
    ...
    create-verticals 1 [                               ; create the vertical lines  by drawing down from the top
      ...
      set heading 180                                  ; aim down
        ifelse x = xLow or abs (x - xHi) < (.001 * abs xHi)  ; the second calculation is required because x might be slightly different than xHi after adding dxx several times. 
          [set color white set pen-size 2.5 ]            ; for the edges!
          [set color gray set pen-size 1.5 ]             ; for the inside lines
        setxy (xm * x + xc) vmax 
        ]
...
to draw-graph ; draws a position or velocity graph depending on vertical-axis-type using existing dots
  ...
  if vertical-axis-type = "Position (m)" [
    set min-y 0  set max-y old-max-y
    ]
  ...
  ask dots with [run-n = run-number ] [   st           ; if there are any dots, set them according to their position or velocity
    ifelse vertical-axis-type = "Position (m)" [place x-val y-val] [place x-val velocity]]
end

to erase-graphs
  ask dots [ht]
  ask lines [set hidden? true]
  set enter-a-run-number 0  ; this tweak allows the current run number to be entered
  set old-enter-a-run-number enter-a-run-number 
  tick
end

to pick-a-question ; ask user to select a question. Wait until a valid choice is made and then unlock the variables. 
  if force-question? [                          ; force question is a author control
    set the-question "To continue, use the arrows at the right to pick a question"
    while [the-question = "To continue, use the arrows at the right to pick a question"] [
      set the-question user-one-of "Select a question" [
      "To continue, use the arrows at the right to pick a question"
      "Just exploring"
      "What is the safe range of driver distances?"
      "What is the safe range of collision speeds?"
      "What range of driver distance and collision speeds represent the 'danger zone'?"
      "What makes more difference for the driver’s safety: driver height or collision speed?"
      "What airbag deployment distance best keeps short drivers safe?"
      "What airbag deployment distance best keeps tall drivers safe?"
      "What airbag deployment time best keeps drivers safe in high speed collisions?"
      "What airbag deployment time best keeps drivers safe in low speed collisions?"]]]
  set variables-locked? false    ; unlock the variables so the user can set them. 
end

  

to run-airbag ; computes and draws the position or velocity graphs of the airbag and dummy 
  if variables-locked? and force-question? [ ; if the variables are locked, then the user has not selected a question. 
    pick-a-question stop]    ; select a question and abort run, waiting for the user to change variables
  ask dots [ht]     ; remove all prior graphs (Kills the lines between them, too)
  ask lines [die]
  tick  
  
  set variables-locked? true
  reset-old-variables          ; ensures that the old values are up-to-date
  set DataSeries "[ ]"        ; this will contain triplet dummy values in json format
  set running? true          ; avoids popping up a question selector if the user changes variables during a run.
  set run-number run-number + 1 ; increment the run counter 
  set enter-a-run-number run-number    ; set the chooser so that it shows the current run.
  let d-color white ; selects the color for this run
  ask actors with [location = "stage"]  [st]
  set time-to-stop max-time-to-stop * car-speed / 40   ; the time to stop the car after collision is proportional to speed. 
  
  ; package up the variables used in analyzing student patterns
  if run-number > 1 [  ; Igore data storage at the start of the first run. 
    ...
    set run-data lput temp-data run-data  ]   ; append this list of results for the prior run to the end of run-data
  ...
  ; the dummy position is determined by solving F=ma for the psudo force of the accelerated reference frame, 
  ...
  let a-max 0 ; will be used to calculate the maximum real acceleration of the dummy
  let a-psudo 0 - (car-speed / time-to-stop)      ; this is the apparent acceleration of the dummy due to the car crash
  set w 0         ; a global used in drawing the lines between current and past dots of the dummy graph (not the bag's graph)
  ...
  while [t <= experiment-duration-in-sec][        ; begin the calculation loop
    ...
    set a-dum 0 - dum-friction * v-dum / time-to-stop      ; add in some unitless 'friction' supplied by the dummy
    if x-dum < x-bag [                             ; if the dummy is inside the bag...
      set a-dum a-dum + const * ( 1 - (x-dum / x-bag ))]   ; add in the force of the airbag
    if abs a-dum > a-max [set a-max abs a-dum]     ; track the maximum real acceleration (not counting the pseudo acceleration
    if t < time-to-stop 
      [set a-dum a-dum + a-psudo ] ; add the psudo-acceleration due to the collision

      ; a-dum is now the sum of these three accelerations--apply a simple Euler solution for v and x
    ...
    set running? false  ; turn this off so data-series is not read when invalid. 
    ...
    update-dummy t x-dum v-dum blue   ; place the dummy and add a dot to the graph
    update-bag t x-bag v-bag blue   ; place the bag and add a dot to the graph (the bag is white--its graph is blue)
  ...
  show-results a-max       ; updates the output box for this run. A-max is in mks units.
  let clr status-color a-max
  ask dots with [run-n = run-number][set dot-color clr set color clr] ; recolor the dots red-yellow-green, depending on the outcome
  ask lines with [runn = run-number][set color clr] 
  set running? false      ; now changes in the variables wil force user to select a question
  output-print dataSeries
end

to reset-old-variables  ; needed because my software variable lock sometimes allows changes
...
to update-dummy [t x-dum v-dum d-color]  ; places a point on the graph and puts the dummy on the stage
  create-dots 1 [          ; create a dummy graph dot for time=t
    ...
    set selected? true  ; probably unnecessary 
    if vertical-axis-type = "Position (m)" [
...
to update-bag [t x-bag v-bag d-color]  ; puts a point on the graph and places the bag
  ; the bag's color is white but the dot it draws is the color for this run
  create-dots 1 [ 
    set x-val t 
    set y-val x-bag
    set velocity v-bag
...
to-report dummy-status [a-max]  ; returns yes, maybe, or no depending on how close am is to a-cutoff. 
  let am a-max / 9.81  ; convert to 'g' units
  if am < .9 * a-cutoff  [report "Yes"]  ; give a 10% margin 
  if am > 1.1 * a-cutoff  [report "No"]
  report "Maybe"
end

to-report status-color [a-max]
  let am a-max / 9.81 ; convert to 'g' units
  if am < .9 * a-cutoff  [report green]  ; give a 10% margin 
  if am > 1.1 * a-cutoff  [report red]
  report yellow
end
...
to show-results [a-max]  ; generates a line in the output box summarizing a run
  ...
  output-type (pad (word run-number) 7)
  
  let am (precision (a-max / 9.8) 1 )   ; convert max acceleration to g units rounded to one decimal
  output-type pad dummy-status a-max 9
  

  let temp word am "g"   ; max acceleration in g units
  ...
  ; now store the parameter data for this run as a list that will be added to the lists in run-data
  set temp-data (list car-speed distance-to-steering-wheel airbag-size time-to-fill-bag am (dummy-status a-max)) 
  
  ; finally, create one dot2 for the parameter graph
  create-dots2 1 [
    set color status-color a-max
...
to-report for-sure?
  report true   ; remove this--this question is very annoying when testing.
  report user-yes-or-no? "Are sure that you want to erase all your data?"
end

to switch-y-axis
  let temp vertical-axis-type
  if temp = "Position (m)" [set vertical-axis-type "Velocity (m/s)" set old-max-y max-y]
  if temp = "Velocity (m/s)" [set vertical-axis-type "Position (m)" set max-y old-max-y]
  ask dots [ht]   
  draw-graph
  set graph-type-used 3 ; if the student changes the axis, this reports that both types were used
end
...
to-report bag-loc [t]   ; reports the location of the airbag front relative to the steering wheel
  let t0 t-delay             ; the time at which the bag starts to inflate
  let tf t0 + time-to-fill-bag    ; time when th bag is completely filled
  let s airbag-size
  if t < t0 [report 0]                       ; start at zero
  if t < tf [report s * (t - t0) / (tf - t0)]       ; linear filling
...
to-report bag-vel [t]     ; reports the velocity of the airbag front relative to the steering wheel
  let t0 t-delay           ; the time at which the bag starts to inflate
  let tf t0 + time-to-fill-bag    ; time when th bag is completely filled
  let t3 t0 + deflate-time * time-to-fill-bag   ; the bag deflates 1/2 as fast as it fills
  let s airbag-size
  if t < t0 [report 0]
  if t < tf [report s / (tf - t0) ]
  if t < t3 [report -1 / (t3 - t0)]  ; linear deflation after it fills, reaching zero at three fill times
  report 0
end

to-report deflate [t]   ; models the deflation of the bag. Multiplies the bag size
  if not deflate?  [ report 1 ]
  let t0 time-to-fill-bag + t-delay  ; deflation starts as soon as the bag is filled = t0
  let t3 t0 + deflate-time * time-to-fill-bag   ; the bag deflates 1/2 as fast as it fills
  if t < t0 [report 1]  ; no deflation for times before it fills
  if t < t3 [report (t3 - t) / (t3 - t0)]  ; linear deflation after it fills, reaching zero at three fill times
  report 0
end
...
to show-run
  ...
  if n > 0 and n <= run-number [
    ask dots with [run-n = n ][
      ...
      ; reconnect the dots
      if prior-who > 0 [
        create-line-with dot prior-who [
          set hidden? false
          set thickness .6
          set color clr]]]
...
to flush     ; normally, the data for a run is computed and saved only when the next run starts--this routine adds the last user run data post-run. 
             ;    saving data at the start of run-airbag leaves the run-data incomplete when leaving the model, so this routine completes run-data. 
  set temp-data sentence temp-data (list graph-type-used timer the-question) 
  set temp-data lput prior-runs-viewed temp-data    ; prior-runs is a list, so needs to be tacked on separately
...
;  if the-question = "What airbag deployment time best keeps drivers safe in low speed collisions?" [set correct-variables [1 4]]
  ...
  let repeats 0              ; the number of times a run was repeated with no changes of variables
  let one-changed 0; This will be the number of times only one variable was changed between successive runs
  let CoV-repeats 0 ; This will be the number of times the same variable was changed between successive runs. 
  let rd run-data
  while [length rd > 1] [   ; at least one pair of runs is needed
    ...
    let old-run sublist (first rd ) 0 4                    ; ditto for the run before--the old run
                    ; match generates a list of 4 values: 0 if the two corresponding elements are the same, 1 otherwise
    let changes reduce + same? current-run old-run         ; changes is the number of variables changed
    ...
    if changes = 1 [                                       ; if only one variable was changed....
      set one-changed one-changed + 1                      ;   increment a one-change
      if position 1 current-run = position 1 old-run [     ; if the same variable was the one changed...
        set CoV-repeats CoV-repeats + 1  ]]                ;   increment CoV-repeats
    set rd bf rd ]                                         ; chop off the first run, get ready to process the next pair
  if run-number = 0 [stop]
  output-print "***************"
  output-print word "Total number of runs: " run-number
  let temp round (100 * Repeats / run-number )
  output-print (word "Number of repeated runs: " repeats " (" temp "%)")
  set temp round (100 * one-changed / run-number)
  output-print (word "Number of runs with only one variable changed: " one-changed " (" temp "%)")
  set temp round (100 * CoV-repeats / run-number )
  output-print (word "Number of runs with the same variable changed: " CoV-repeats " (" temp "%)")
end
...
to-report min-max [name] ;
  ...
  if name = "Airbag size" [report list 0 .5]
...
to make-ModelDataDescription
  set data-ready? false
  ; this puts into the global ModelDataDescription something like the following
  ; {
  ;   "DataSeriesDescription": [
  ...
  set temp (word temp "{\n")
  set temp (word temp "  \"DataSeriesDescription\": [\n")
  ...
  set ModelDataDescription temp
  output-print ModelDataDescription
  set data-ready? true
end
@#$#@#$#@
GRAPHICS-WINDOW
...
1744
118
Const sets the stiffness of the airbag
11
0.0
1
...
1533
219
Max-time-to-stop is the time that the car takes to stop going 40m/s. The actual time to stop is proportional to speed. Longer times are due to built-in crumpling. Short times would apply to rigid cars. 
11
0.0
1
...
1575
360
Deflate? controls whether the airbag deflates after deploying. Deflate-time determines how long the bag takes to deflate
11
0.0
1
...
996
174
Enter a run number to be graphed.
11
0.0
0
