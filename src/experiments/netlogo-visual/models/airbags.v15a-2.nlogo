...
to startup
  ...
  reset-timer 
  output-print (word "Run Color  Survives   Max-acc  Car speed   Distance  Bag size  Fill time")
  set dummy-colors ["red" "orange" "green" "cyan" "yellow" "magenta" "gray" "brown" "lime" "turquoise" "sky" "violet" "pink" "blue"] 
  set run-number 0
...
to run-airbag ; computes and draws the position or velocity graphs of the airbag and dummy 
  ...
  let temp (run-number - 1) mod 13  ; there are 13 named colors--cycle through them
  let d-color read-from-string (item temp dummy-colors)  ; selects the color for this run
  ...
  ; force user to select a question
  ...
  let a-max 0 ; will be used to calculate the maximum acceleration of the dummy
  ...
  while [t <= experiment-duration-in-sec][        ; begin the calculation loop
    ...
    ifelse t < time-to-stop 
      [set a-dum 0 + a-psudo ] ; the psudo-acceleration due to the collision
      [set a-dum 0 ]                              ; zero acceleration due to the collision after time-to-stop
    if x-dum < x-bag [                             ; if the dummy is inside the bag...
      set a-dum a-dum + const * ( 1 - (x-dum / x-bag ))]   ; add in the force of the airbag
    set a-dum a-dum - dum-friction * v-dum / time-to-stop      ; add in some unitless 'friction' supplied by the dummy
    if abs a-dum > a-max [set a-max abs a-dum]     ; track the maximum acceleration
...
to update-bag [t x-bag v-bag d-color]  ; puts a point on the graph and places the bag
  ...
  create-dots 1 [ 
    ...
    set y-val x-bag
    set velocity v-bag
    set dot-color d-color   ; the graph shows the color of this run--the d-color
    set run-n run-number
...
to show-results [a-max]       ; generates a line in the output box summarizing a run
  ...
  output-type pad item temp dummy-colors 10
...
1658
357
Deflate? controls whether the airbag deflates after deploying. Deflate-time determinse how long the bag takes to deflate
11
0.0
1
