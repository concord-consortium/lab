...
const lines = aminoacidsProps.split("\n");
const aminoProps = [];
const propNames = [];

for (let line of lines) {
  if (line.length === 0) { continue; }
  const names = line[0] === "n" ? true : false;
  ...
  if (names) {
    for (i = 0; i < props.length; i++) {
      prop = props[i];
      if (prop === "Full name") { prop = "fullName"; }
      if (prop === "Hydrophobicity (RB)") { prop = "hydrophobicityRB"; }
      if (prop === "Molecular weight") { prop = "molWeight"; }
