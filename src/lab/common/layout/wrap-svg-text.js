  ...
  return wrapSVGText = (window.wrapSVGText = function(text, svgTextNode, maxWidth, x, dy) {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if ((i === 0) || newlinemode || ((maxWidth > 0) && (computedTextLength > maxWidth))) {    // create new tspan
        ...
        if (~dashArray.indexOf(curLineLength-1)) { line = word + "-";
        } else { line = word + " "; }

        if (i && lastWord) { line = lastWord + line; }
    ...
    if (widestWidth === 0) { widestWidth = svgTextNode.childNodes[0].getComputedTextLength(); }
    ...
    return {lines: numLines, width, textWidth: widestWidth};
  });});