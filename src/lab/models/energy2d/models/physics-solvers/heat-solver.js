    ...
    macCormack = function(t) {
      ...
      for (i = 1; i < nx1; i += 1) {
        ...
        for (j = 1; j < ny1; j += 1) {
          ...
          jinx_plus_1 = jinx + 1;
          if (fluidity[jinx]) {
            t0[jinx] = t[jinx] - tx * (u[jinx_plus_nx] * t[jinx_plus_nx] - u[jinx_minus_nx] * t[jinx_minus_nx]) - ty * (v[jinx_plus_1] * t[jinx_plus_1] - v[jinx_minus_1] * t[jinx_minus_1]);
          }
        }
      }
      applyBoundary(t0);
      ...
      for (i = 1; i < nx1; i += 1) {
        ...
        for (j = 1; j < ny1; j += 1) {
          ...
          if (fluidity[jinx]) {
            ...
            t[jinx] = 0.5 * (t[jinx] + t0[jinx]) - 0.5 * tx * u[jinx] * (t0[jinx_plus_nx] - t0[jinx_minus_nx]) - 0.5 * ty * v[jinx] * (t0[jinx_plus_1] - t0[jinx_minus_1]);
          }
        }
      }
      applyBoundary(t);
        ...
        for (i = 1; i < nx1; i += 1) {
          ...
          for (j = 1; j < ny1; j += 1) {
            ...
            if (isNaN(tb[jinx])) {
              ...
              t[jinx] = (t0[jinx] * sij + q[jinx] + axij * t[jinx_minus_nx] + bxij * t[jinx_plus_nx] + ayij * t[jinx_minus_1] + byij * t[jinx_plus_1]) / (sij + axij + bxij + ayij + byij);
            } else {
              t[jinx] = tb[jinx];
            }
          }
