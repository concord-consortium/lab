...
export const makeFluidSolver = function(model) {
  var
        ...
        for (i = 1; i < nx1; i += 1) {
          ...
          for (j = 1; j < ny1; j += 1) {
            ...
            if (fluidity[jinx]) {
              ...
              phi[jinx] = s * ((phi[jinx_minus_nx] + phi[jinx_plus_nx]) * idxsq + (phi[jinx_minus_1] + phi[jinx_plus_1]) * idysq - div[jinx]);
            }
          }
        }
      }
        ...
        for (i = 1; i < nx1; i += 1) {
          ...
          for (j = 1; j < ny1; j += 1) {
            ...
            if (fluidity[jinx]) {
              ...
              f[jinx] = (f0[jinx] + hx * (f[jinx_minus_nx] + f[jinx_plus_nx]) + hy * (f[jinx_minus_1] + f[jinx_plus_1])) * dn;
            }
          }
        }
        applyBoundary(b, f);
      ...
      for (i = 1; i < nx1; i += 1) {
        ...
        for (j = 1; j < ny1; j += 1) {
          ...
          if (fluidity[jinx]) {
            ...
            f[jinx] = f0[jinx] - tx * (u0[jinx_plus_nx] * f0[jinx_plus_nx] - u0[jinx_minus_nx] * f0[jinx_minus_nx]) - ty * (v0[jinx_plus_1] * f0[jinx_plus_1] - v0[jinx_minus_1] * f0[jinx_minus_1]);
          }
        }
      }
      ...
      for (i = 1; i < nx1; i += 1) {
        ...
        for (j = 1; j < ny1; j += 1) {
          ...
          if (fluidity[jinx]) {
            ...
            f0[jinx] = 0.5 * (f0[jinx] + f[jinx]) - 0.5 * tx * u0[jinx] * (f[jinx_plus_nx] - f[jinx_minus_nx]) - 0.5 * ty * v0[jinx] * (f[jinx_plus_1] - f[jinx_minus_1]);
          }
        }
      }
