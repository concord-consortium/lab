  ...
  */
  convertKEtoT = function(totalKEinMWUnits, N) {
    ...
    var averageKEinMWUnits = totalKEinMWUnits / N,
      averageKEinJoules = constants.convert(averageKEinMWUnits, {
        from: unit.MW_ENERGY_UNIT,
        to: unit.JOULE
      });
  ...
  */
  convertTtoKE = function(T, N) {
    var averageKEinJoules = T * BOLTZMANN_CONSTANT_IN_JOULES,
      averageKEinMWUnits = constants.convert(averageKEinJoules, {
        from: unit.JOULE,
        to: unit.MW_ENERGY_UNIT
      }),
...
export const createEngine = function() {
    ...
    */
    ...
    updateRadialBondForces = function() {
      ...
      for (i = 0; i < N_radialBonds; i++) {
        ...
        fOverR = constants.convert(k * (r - r0), {
          from: unit.EV_PER_NM,
          to: unit.MW_FORCE_UNIT
        }) / r;
    ...
    updateRestraintForces = function() {
      ...
      for (i = 0; i < N_restraints; i++) {
        ...
        f_over_r = constants.convert(k * r, {
          from: unit.EV_PER_NM,
          to: unit.MW_FORCE_UNIT
        }) / r;
  ...
  engine = {
    ...
    computeOutputState: function(state) {
      ...
      for (i = 0; i < N; i++) {
        ...
        if (gravitationalField) {
          ...
          PE += constants.convert(gravPEInMWUnits, {
            from: unit.MW_ENERGY_UNIT,
            to: unit.EV
          });
        }
        ...
        for (e = 0; e < N_electricFields; e++) {
          ...
          switch (electricFieldOrientation[e]) {
            case "N":
            case "S":
              elInMWUnits *= (rect != null ? shapeY[rect] : minY) - y[i];
              break;
            case "W":
            case "E":
              elInMWUnits *= (rect != null ? shapeX[rect] : minX) - x[i];
              break;
          }
          PE += constants.convert(elInMWUnits, {
            from: unit.MW_ENERGY_UNIT,
            to: unit.EV
          });
        }
      ...
      state.time = time;
      state.PE = PE;
      state.KE = constants.convert(KEinMWUnits, {
        from: unit.MW_ENERGY_UNIT,
        to: unit.EV
      });
      state.temperature = T;
    ...
    */
    newPotentialCalculator: function(testElement, testCharge, calculateGradient) {
      ...
      return function(testX, testY) {
        ...
        if (calculateGradient) {
          gradX = constants.convert(fx, {
            from: unit.MW_FORCE_UNIT,
            to: unit.EV_PER_NM
          });
          gradY = constants.convert(fy, {
            from: unit.MW_FORCE_UNIT,
            to: unit.EV_PER_NM
          });
    ...
    addKEToAtomPairAndConserveMomentum: function(deltaKE, atom1, atom2) {
      ...
      var newKE = oldKE + constants.convert(deltaKE, {
        from: unit.EV,
        to: unit.MW_ENERGY_UNIT
      });
    ...
    */
    ...
    getCoulombForceAt: function(testX, testY, resultObj) {
      ...
      resultObj.fx = constants.convert(fx, {
        from: unit.MW_FORCE_UNIT,
        to: unit.EV_PER_NM
      });
      resultObj.fy = constants.convert(fy, {
        from: unit.MW_FORCE_UNIT,
        to: unit.EV_PER_NM
      });
      return resultObj;
    ...
    */
    getAtomKineticEnergy: function(i) {
      ...
      return constants.convert(KEinMWUnits, {
        from: unit.MW_ENERGY_UNIT,
        to: unit.EV
      });
    },
