...
export default function MD2DView(modelView, model) {
  ...
  var api = {},
    ...
    velocityVectorsRenderer = new VectorsRenderer(vectorsAbovePixi.pixiContainer, {
      get show() {
        return model.get("showVelocityVectors");
      },
      get length() {
        return model.get("velocityVectors").length;
      },
      get width() {
        return model.get("velocityVectors").width;
      },
      get color() {
        return model.get("velocityVectors").color;
      },
      get dirOnly() {
        return false;
      },
      get count() {
        return modelAtoms.length;
      },
      x: function(i) {
        return modelAtoms[i].x;
      },
      y: function(i) {
        return modelAtoms[i].y;
      },
      vx: function(i) {
    ...
    forceVectorsRenderer = new VectorsRenderer(vectorsAbovePixi.pixiContainer, {
      get show() {
        return model.get("showForceVectors");
      },
      get length() {
        return model.get("forceVectors").length;
      },
      get width() {
        return model.get("forceVectors").width;
      },
      get color() {
        return model.get("forceVectors").color;
      },
      get dirOnly() {
        return model.get("forceVectorsDirectionOnly");
      },
      get count() {
        return modelAtoms.length;
      },
      ...
      vx: function(i) {
        var a = modelAtoms[i];
        return a.ax * a.mass * 100;
      },
      vy: function(i) {
        var a = modelAtoms[i];
        return a.ay * a.mass * 100;
      },
      m2px: modelView.model2canvas,
      m2pxInv: modelView.model2canvasInv
    }),
    electricFieldRenderer = new FieldRenderer(vectorsBelowPixi.pixiContainer, {
      get show() {
        return model.get("showElectricField");
      },
      get length() {
      ...
      x: function(i) {
        return modelElectricField[i].x;
      },
      y: function(i) {
        return modelElectricField[i].y;
      },
      vx: function(i) {
        return modelElectricField[i].fx;
      },
      vy: function(i) {
        return modelElectricField[i].fy;
      },
      alpha: function(i) {
  ...
  function photonColor(data) {
    ...
    if (freq <= MIN_VISIBLE_FREQ || freq >= MAX_VISIBLE_FREQ) {
      ...
      if (i === 6) {
        ...
        return "rgb(" + Math.floor(r1 + remainder * (r2 - r1)) + "," + Math.floor(g1 + remainder * (g2 - g1)) + "," + Math.floor(b1 + remainder * (b2 - b1)) + ")";
      }
    }
  }
