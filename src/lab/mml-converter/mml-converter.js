...
define(function (require) {
  ...
  let parseMML;
  const cheerio = require("browserified-cheerio");
  const constants = require("models/md2d/models/engine/constants/index");
  const {
    unit
  } = constants;
  const validator = require("common/validator");
  const metadata = require("models/md2d/models/metadata");
  const Solvent = require("models/md2d/models/solvent");
  ...
  */
  return parseMML = function (mmlString) {
    ...
    const $mml = cheerio.load(mmlString, {xmlMode: true});
    ...
    const toNumber = function ($node, {defaultValue}) {
      const val = $node.text();
      if (val != null) {
    ...
    const parseObstacles = function () {
      ...
      for (node of Array.from(obstacleNodes)) {
        ...
        const validatedData = validator.validateCompleteness(metadata.obstacle, rawData, {includeOnlySerializedProperties: true});
    ...
    const parseRectangles = function () {
      ...
      for (let idx = 0; idx < rectangleNodes.length; idx++) {
        ...
        // Validate all properties and provides default values for undefined values.
        const validatedData = validator.validateCompleteness(metadata.shape, rawData, {includeOnlySerializedProperties: true});
    ...
    const parseLines = function () {
      ...
      for (let idx = 0; idx < lineNodes.length; idx++) {
        ...
        // Validate all properties and provides default values for undefined values.
        const validatedData = validator.validateCompleteness(metadata.line, rawData, {includeOnlySerializedProperties: true});
    ...
    */
    var parseElectricField = function (node) {
      ...
      const rawData = {intensity, orientation};
      ...
      // Validate all properties and provides default values for undefined values.
      const props = validator.validateCompleteness(metadata.electricField, rawData, {includeOnlySerializedProperties: true});
    ...
    */
    ...
    for (node of Array.from(elementNodes)) {
      ...
      const elementRawData = {mass, sigma, epsilon, color};
      ...
      // Validate all properties and provides default values for undefined values.
      const elementValidatedData = validator.validateCompleteness(metadata.element, elementRawData, {includeOnlySerializedProperties: true});
    ...
    */
    ...
    for (prop of Array.from(lbMixingProps)) {
      ...
      if ($prop.find("boolean").text() === "false") {
        ...
        let ljProps = {element1, element2, sigma, epsilon};
    ...
    const parseAtoms = function () {
      ...
      for (node of Array.from(atomNodes)) {
        ...
        if (restraint.length > 0) {
          ...
          const restraintRawData = {atomIndex, k, x0, y0};
          ...
          // Validate all properties and provides default values for undefined values.
          const restraintValidatedData = validator.validateCompleteness(metadata.restraint, restraintRawData, {includeOnlySerializedProperties: true});

          restraints.push(restraintValidatedData);
        }


        const atomRawData = {element, x, y, vx, vy, charge, friction, radical, pinned, marked, visible, draggable, draggableWhenStopped};
        ...
        // Validate all properties and provides default values for undefined values.
        const atomValidatedData = validator.validateCompleteness(metadata.atom, atomRawData, {includeOnlySerializedProperties: true});
    ...
    */
    ...
    for (node of Array.from(radialBondNodes)) {
      ...
      const radialBondRawData = {atom1, atom2, length, strength, type};
      ...
      // Validate all properties and provides default values for undefined values.
      const radialBondValidatedData = validator.validateCompleteness(metadata.radialBond, radialBondRawData, {includeOnlySerializedProperties: true});
    ...
    */
    ...
    for (node of Array.from(angularBondNodes)) {
      ...
      const angularBondRawData = {atom1, atom2, atom3, angle, strength};

      // Validate all properties and provides default values for undefined values.
      const angularBondValidatedData = validator.validateCompleteness(metadata.angularBond, angularBondRawData, {includeOnlySerializedProperties: true});
