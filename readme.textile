...
h3. Using an earlier version of Energy2D-JS setup specifically as a benchmark [3]

Results from running this "benchmark":https://bug636096.bugzilla.mozilla.org/attachment.cgi?id=514432 

This benchmark was adapted by Mozilla developer Vladimir Vukicevic from an earlier version of Energy2D-JS and runs 100 model iterations with no browser rendering at all. These results should not be directly compared with the results above because the model operation has changed since the FireFox developers adapted Energy2D-JS into a performance benchmark.
...
fn3. See the Mozilla performance bug report: "really slow on earthmodel testcase/benchmark":https://bug636096.bugzilla.mozilla.org -- the benchmark can be run directly "here":https://bug636096.bugzilla.mozilla.org/attachment.cgi?id=514432
