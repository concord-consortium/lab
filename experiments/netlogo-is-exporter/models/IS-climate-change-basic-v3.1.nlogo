...
to startup
  ...
  set beta 1 - alpha
  repeat 5 [add-co2]  ; start out with some CO2
...
to setup-world
  ...
  ask patches [                           ;; set colors of the world
      ...
      update-albedo ] 
  set year 2000
  set n-add-co2 2    ; number of co2 atoms added or subtracted per click
end      
...
to run-heat    ;; advances the heat energy turtles
  set temperature .99 * temperature + .01 * (-7 + .18 * count heat) ;; the temperature is related to the number of heat turtles
  set smooth-temperature beta * smooth-temperature + alpha * temperature
  if year > 2012 [update-plots]  ; give time for the transients to settle down
  ask heat [
    fd .5 * ( random 11 ) / 10
    ...
    if (ycor >= earth-top ) [  ;; if heading back into sky
      ifelse (random 100) < (20 * temperature) and random 20 < 2     ;; select more if it is hot
        [ set breed IR
        set heading 20
        set color magenta 
...
to add-CO2  ;; randomly adds N-add-CO2 molecules to atmosphere
  let width sky-top - earth-top
...
"" ""
PENS
"default" 1.0 0 -2674135 true "" "if year > 2010 [plotxy year temperature]"
"avg" 1.0 0 -16448764 true "" "if year > 2010 [plotxy year precision Smooth-temperature 1]"
