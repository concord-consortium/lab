...
// - **graphs**: a graph objects to send data to
...
sensor.AppletGrapher = function(applet, container, graphs, sensorConfig, sampleInterval, listener_str, appletReadyCallback) {
  this.applet = applet;
  ...
  return {
    ...
    dataReceived: function(type, count, data) {
      if (type === 1000) {
        ...
        for(i= 0; i < count; i++) {
          for (j= 0; j < sensors; j++) {
            newdata[j].push(data[sensors*i + j]);
          }
