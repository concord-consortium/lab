...
globals [
  ...
  old-in-grid?
  old-in-grid2?
  old-y-axis               ; used for the main graph
...
to initialize
 ...
 create-messages 1 [
   ...
   set color 18
   setxy .65 * max-pxcor .97 * max-pycor ht]
...
to act-on-changes   ; look for changes in the sliders and selectors and takes appropriate action 
  ...
  if old-vertical-axis != vertical-axis [
    set old-vertical-axis vertical-axis  draw-view ]  ; 
  
  if old-horizontal-axis != horizontal-axis [
    set old-horizontal-axis horizontal-axis draw-view ] ; 
...
to-report cursor-near? [n]               ; reports true if the cursor is near the dot for run n in grid2
  ...
  let x first [xcor] of dots2 with [label = n]  ; the label of a dot2 contains its run number!
  let y first [ycor] of dots2 with [label = n]
...
to run-airbag ; computes and draws the position or velocity graphs of the airbag and dummy 
  ...
  if what-is-your-question? = "" and not omit-question? [
  ...
  let dt experiment-duration-in-sec / 120        ; results in 120 points, which seems continuous. Controls the speed of the animation. 
  ...
  while [t <= experiment-duration-in-sec][       ; begin the calculation loop
    ...
    if x-dum <= 0 [                           ; if the dummy hits the steering wheel (at x=0)
      if not dummy-crashed? [                 ; if this is the first time the dummy crashes
        let temp (0 - v-old) / dt             ; set a very high acceleration
  ...
  ; save run data
  show-results          ; updates the output box for this run
...
to recolor-dummy-graph    ; colors graph run-number red, yellow, or green depending on a-max
  ...
  let c yellow
  if a-max-g < .9 * a-cutoff  [set c green]  ; give a 10% margin 
  if a-max-g > 1.1 * a-cutoff  [set c red]
  ...
  ask dots with [run-n = run-number and color = cyan ][  ; select current bag points
    ...
    ; now change any dots that are at the maximum acceleration
    let a-temp acceleration                             ; the real acceleration of this dot in mks units
    if a-max = 0 [set a-max 1e-6]
    if abs ((a-temp - a-max) / a-max) < .02 [            ; if this dot is withing 2% of the max, make it bigger
      set a-max? true
      set size 3 ]]
  ask lines with [run-num = run-number and color = cyan][
    set color c ]
end
...
to-report bags-force [t x-dum v-dum dt]          ; calculate the bag's dynamics: x-bag, v-bag, and the bag's force on the dummy given the dummy at x-dum
  ...
  if dummy-crashed? [                            ; if the dummy has already crashed into the steering wheel... return a force of zero
...
to update-run-data 
  ...
  ;   The variables above are computed at the end of the run ans are stored as a list in temp-data
  ...
  set temp-data lput timer temp-data    
  set temp-data lput prior-runs-viewed temp-data    ; prior-runs is a list, so needs to be tacked on separately
  set temp-data lput run-groups-viewed temp-data    ;
  set temp-data lput used-cursor? temp-data    ;
  set temp-data lput cursor-times temp-data    ;
  set temp-data lput cursor-time temp-data    ;
  set temp-data lput used-pointer? temp-data    ;
  set temp-data lput pointer-times temp-data    ;
  set temp-data lput pointer-time temp-data    ;
  set temp-data lput hover-times temp-data    ;
  ; at this point, temp-data contains everything we want to know about the run just completed
  set run-data lput temp-data run-data     ; append this list of results to the end of run-data
  ; run-data is a list of lists, each one of which contains all the data from one run
end

to show-results       ; generates a line of text in the output box summarizing a run
    ...
    create-verticals2 1 [                               ; create the vertical lines  by drawing down from the top
      ...
      set heading 180                                  ; aim down
        ifelse x = xLow or x = xHi 
          [set color white set pen-size 2 ]            ; for the edges!
          [set color 7 set pen-size 1 ]             ; for the inside lines
        setxy (xm * x + xc) vmax 
        ]
     set x x + dxx ]                                     ; at this point turtles are poised to descend from the top of the graph
...
1333
65
Use these to 'tune up' the model. They will not be in the final model. 
11
0.0
1
...
1326
226
Time-to-stop-going-40 is the time that the car takes to stop going 40m/s. The actual time to stop is proportional to speed. Longer times are due to built-in crumpling. Short times would apply to rigid cars. 
11
0.0
1
...
1816
459
t-delay is the delay between the beginning of the crash and the start of airbag deployment\na-cutoff is the max safe acceleration\nbag-mult determines the apparent size of the bag
11
0.0
1
...
1814
539
Dum-friction determines the force that the human supplies that opposes the motion
11
0.0
1
...
1567
74
The stiffness when the airbag is inflating. 
11
0.0
1
...
1558
142
The stiffness of the fully inflated bag.
11
0.0
1
...
1815
587
Penetration is the distance the dummy can penetrate the airbag
11
0.0
1
...
1390
294
This sets the effective mass of the dummy.
11
0.0
1
...
Dummy-size
Dummy-size
"Baby" "Child" "Normal Adult" "Overweight Adult"
2
